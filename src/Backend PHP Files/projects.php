...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $projectId = mysqli_real_escape_string($con, $input['id']);

    $deleteLinksSql = "DELETE FROM project_member_links WHERE project_id = ?";
    $stmtLinks = mysqli_prepare($con, $deleteLinksSql);
    mysqli_stmt_bind_param($stmtLinks, "i", $projectId);
    ...
    if (mysqli_stmt_execute($stmtProject)) {
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error deleting project: " . mysqli_error($con)]);
    }
